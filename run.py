...
for strOption, strArgument in getopt.getopt(sys.argv[1:], '', [ strParameter[2:] + '=' for strParameter in sys.argv[1::2] ])[0]:
	...
	if strOption == '--data_path' and strArgument != '': data_path = strArgument # path to the data directory
# end
...
if __name__ == '__main__':
	sample_list = [os.path.join(data_path, o) for o in os.listdir(data_path) if os.path.isdir(os.path.join(data_path,o))]
	for sample_path in tqdm(sample_list, ascii = True):
		sample_flow = {}
		with h5py.File(os.path.join(sample_path, f'frames.hdf5'), "r") as frames_sample:
			amb = torch.tensor(frames_sample['ambient'])
			for tidx0 in range(amb.shape[0]):
				for tidx1 in range(0, amb.shape[0]):
					if tidx0 != tidx1:
						tenFirst = amb[tidx0].expand(3, -1, -1)
						tenSecond = amb[tidx1].expand(3, -1, -1)
						sample_flow[f'flow_{tidx0}{tidx1}'] = tenOutput = estimate(tenFirst, tenSecond)[None]

		with h5py.File(os.path.join(sample_path, f'flow.hdf5'), "w") as f:
			for k, val in sample_flow.items():
